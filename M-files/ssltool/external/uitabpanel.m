...
% UITABPANEL Add a group of tabbed uipanel container objects to a figure.
...
% Modified by Siyi Deng; 04-01-2011;
...
for i = 1:2:nargin
  ...
  switch property
    case {'style','tabposition','titleposition'}
      style = value;
      type = find(strcmpi(style,styles));
      if isempty(type)
...
if type==1
  ...
  for i = 1:ntab
    ...
    panel(i) = uipanel( ...
      'Parent',htab, ...
      'Units','pixels', ...
      'Position',titlepos+[0,0,0,panelheight(i)*charsz(4)], ...
  ...
  for i = 1:ntab
    title(i) = uipanel( ...
      ...
      'BackgroundColor',titlebackgroundcolor, ...
      'BorderType',panelbordertype, ...
      'ButtonDownFcn',{@TopBottomTabCbk,i}, ...
      'Tag',sprintf('TabBase%02d',i),...
      'Visible','on');
...
%--------------------------------------------------------------------------
  function PopupTabCbk(~,evdt,indx)
    ...
    if ~strcmpi('on',get(panel,'Visible'))
...
%--------------------------------------------------------------------------
  function PopupTabSliderCbk(hobj,~)
...
%--------------------------------------------------------------------------
  function PopupTabResizeCbk(~,evdt)
...
%--------------------------------------------------------------------------
  function TopBottomTabCbk(hobj,~,indx)
...
%--------------------------------------------------------------------------
  function TopBottomTabResizeCbk(~,~)
