...
%   %%% RR: removed done previosuly detrend - 1 or 0 (default 1): 1 means detrending data before fft, otherwise 0
...
function spec = irasaRR(sig,srate,param,varargin)
...
fmax = srate/2; 
flag_filter = 1;
eval(['hset=[',param.hset,'];'])
%%% hset=1.1:0.05:1.9;

if strfind(param.irasaSpectraType,'power')
    spectExp=2; 
elseif strfind(param.irasaSpectraType,'amp')
    spectExp=1;
else 
    error('IRASA: neither power nor amplitude') 
end 
...
[Smixd, Sfrac, freq] = irasa(sig,srate,spectExp,param.nSubset,param.irasaWindowType,hset,flag_filter,param.fftNorm2Hz);  
...
function [Smixd, Sfrac, freq] = irasa(sig,srate,spectExp,Nsubset,irasaWindowType,hset,flag_filter,fftNorm2Hz)
...
if strcmp(irasaWindowType,'none')
    taper = [];
else
    taper = gettaper([Ndata dim],irasaWindowType);
end
for k = 0:Nsubset-1
    i0 = L*k+1;
    x1 = sig(i0:1:i0+Ndata-1,:);
    if isempty(taper)
        p1 = fft(x1,nfft);        % /min(nfft,size(x1,1));
    else
        p1 = fft(x1.*taper,nfft); % /min(nfft,size(x1,1));
    end
    if strcmp(fftNorm2Hz,'yes')
        p1=p1/min(nfft,size(x1,1));
    end
    p1(2:end,:) = p1(2:end,:)*2;
    Smixd = Smixd+abs(p1(1:Nfrac,:)).^spectExp;
end
Smixd = Smixd / Nsubset;
...
for ih = 1:length(hset)
    ...
    for k = 0 : Nsubset-1
        ...
        if strcmp(irasaWindowType,'none')
            ...
            taperh = gettaper(size(xh),irasaWindowType);
        end
        if isempty(taperh)
            ph = fft(xh,nfft);         % /min(nfft,size(xh,1));
        else
            ph = fft(xh.*taperh,nfft); % /min(nfft,size(xh,1));
        end
        if strcmp(fftNorm2Hz,'yes')
            ph=ph/min(nfft,size(xh,1));
        end
        ph(2:end,:) = ph(2:end,:)*2;
        tmp = (abs(ph)).^spectExp;
        Sh = Sh + tmp(1:Nfrac,:);   
    ...
    for k = 0 : Nsubset - 1
        ...
        if strcmp(irasaWindowType,'none')
            ...
            taper1h = gettaper(size(x1h),irasaWindowType);
        end
        if isempty(taper1h)
            p1h = fft(x1h,nfft);          % /min(nfft,size(x1h,1));
        else
            p1h = fft(x1h.*taper1h,nfft); % /min(nfft,size(x1h,1));
        end
        if strcmp(fftNorm2Hz,'yes')
            p1h=p1h/min(nfft,size(x1h,1));
        end
        p1h(2:end,:) = p1h(2:end,:)*2;
        tmp = (abs(p1h)).^spectExp;
        S1h = S1h + tmp(1:Nfrac,:);
    end
    S1h = S1h / Nsubset;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function taper = gettaper(S,irasaWindowType)
% get a tapering function for power spectrum density calculation
switch irasaWindowType
    case 'hamming'
